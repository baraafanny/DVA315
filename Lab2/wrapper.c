...
HANDLE mailslotCreate (char *name) {
	...
	//Namn på sloten, Hur stort meddelande den ska ta 0 för vilken storlek som helst, 0 Returns immediately if no message is present, NULL för optional.
	char name2[100];
...
HANDLE mailslotConnect (char * name) {
	...
	//Namnet på filen, Skriv rättigheter, Man kan senare skriva till filen, Optional, Gör en ny fil alltid när detta anropas, Vanligaste valet, Optional.
	char name2[100];
	char* name3 = "\\\\.\\mailslot\\%s";
	sprintf_s(name2, 100, name3, name);
